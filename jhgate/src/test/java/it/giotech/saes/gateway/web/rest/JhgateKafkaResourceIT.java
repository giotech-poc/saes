...
import it.giotech.saes.gateway.config.KafkaProperties;
...
class JhgateKafkaResourceIT {
    ...
    void setup() {
        ...
        JhgateKafkaResource kafkaResource = new JhgateKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhgate-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jhgate-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
